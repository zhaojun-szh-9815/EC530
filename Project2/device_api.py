...
def select_related(id):
    ...
    if r[1] == id:
        ...
        for row in rows:
            person = dict()
            message = select_user(row[2])
            person["ID"] = message["U_ID"]
            person["First_name"] = message["First_Name"]
            ans[f"{i}"] = person
            i += 1
        resp["Content"] = ans
    if r[2] == id:
...
def check_user_password(username, password):
    ...
    conn.create_function("check_role", 1, sqlite_custom_function)
    cur = conn.cursor()
    cur.execute("SELECT U_ID, Password FROM users where Username = ?", (username,))
    rows = cur.fetchall()
    ans = dict()
    try:
        row = rows[0]
        ...
        ans["State"] = "Username Incorrect"
        ans["Content"] = None
        return ans
    
    if not password == row[1]:
        ans["State"] = "Password Incorrect"
        ans["Content"] = None
    else:
        ans["State"] = "Success"
        ans["Content"] = select_user(row[0])
    return ans
...
api.add_resource(User_Password, '/user_password/<username>/<password>')
